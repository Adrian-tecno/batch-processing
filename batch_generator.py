...
def create_data(locale: str) -> Faker:
    logging.info(f"Created syntheetic data for {locale.split('_')[-1]} country code.")
    return Faker(locale)
...
#Logging starting of the process.
    ...
    output_file = fr"D:\Usuario_Adrian_{date.today()}.csv"
    ...
    if str(date.today()) == "2026-05-19":
        records = random.randint(100_000, 125_000)
        run_type = "first"
        
    else:
        records = 100_372 + random.randint(0, 1_101)
        run_type = "next"
